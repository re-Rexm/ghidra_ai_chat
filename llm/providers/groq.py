...
class GroqClient(LLMClientInterface):
    """
    Minimal Groq adapter using the OpenAI-compatible chat completions endpoint.
    """
    ...
    def chat(self, messages, settings):
        ...
        if not api_key:
            raise LLMError(
                "Missing API key. Add key text to '%s' (preferred for per-session use) "
                "or set environment variable %s."
                % (
                    getattr(settings, "api_key_file_path", "<session key file>"),
                    getattr(settings, "api_key_env_var", "GROQ_API_KEY"),
                )
            )
        ...
        if code < 200 or code >= 300:
            ...
            try:
                ...
                err_body = ""
            raise LLMError("Groq HTTP %s: %s" % (code, err_body or "<no body>"))

        body = _read_all(conn.getInputStream())
        try:
            obj = json.loads(body)
        except Exception as e:
            raise LLMError("Invalid JSON from Groq: %s\nRaw:\n%s" % (str(e), body))

        try:
            return obj["choices"][0]["message"]["content"]
        except Exception:
            raise LLMError("Unexpected Groq response shape:\n%s" % body)
